...
public class LadderGameActivity extends ActionBarActivity {
    ...
    final static int border_lbr = R.drawable.border_left_bottom_right;
    final static int border_lr = R.drawable.border_left_right;
    ...
    public void layout_set(){
        ...
        btn_result.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ListViewArrayAdapter listAdapter = new ListViewArrayAdapter(getApplicationContext(), R.layout.result_item, R.id.res_name, result_list_item);
    ...
    */
    ...
    /*
            사다리를 그려주는 함수
        - Bottom 이 있는 사다리 Drawable -> border_lbr
        ...
        if(rows == lines-1) {
            ...
            switch(where) {
                ...
                case MAIN_SET:
                    ...
                    if (set_border == border_lbr)
                        cnt_chk_border[cols] = cnt_chk_border[cols] + 1;
                    break;
                // 현재 행의 오른쪽 열이 border_lbr일 경우, 현재 행은 border_lr를 그려주어 중복을 피함
                case CHK_SET:
                        if (persons != 2 && Integer.parseInt(tv_cols[rows][cols + 1].getTag().toString()) == border_lbr) {
                            tv.setBackgroundResource(border_lr);
                            tv.setTag(border_lr);
                        } else {
                            tv.setBackgroundResource(set_border);
            ...
            switch(where){
                case MAIN_SET:
                    if (Integer.parseInt(tv_cols[rows][cols - 1].getTag().toString()) == border_lbr) {
                        tv.setBackgroundResource(border_lr);
                        tv.setTag(border_lr);
                    } else {
                        tv.setBackgroundResource(set_border);
                ...
                case CHK_SET:
                    if(cols+1 >= tv_cols[rows].length) {
                        ...
                        if (Integer.parseInt(tv_cols[rows][cols - 1].getTag().toString()) == border_lbr || Integer.parseInt(tv_cols[rows][cols + 1].getTag().toString()) == border_lbr) {
                            tv.setBackgroundResource(border_lr);
                            tv.setTag(border_lr);
                        } else {
                            tv.setBackgroundResource(set_border);
                ...
                if (persons_cols == persons - 2) {
                    if (tv_cols_tag == border_lbr) {
                        ...
                        if (vec_switch) {
                            ...
                            if (tv_cols_tag_left == border_lbr) {
                                persons_cols--;
                            }
                        }
                    }
                    ...
                    if (tv_cols_tag == border_lbr) {
                        ...
                        if (!vec_switch) {
                            ...
                            if (tv_cols_tag_right == border_lbr) {
                                persons_cols++;
                            }
                        }
                    }
                } else {
                    // 현재 사다리가 border_lbr가 아닌 경우, 좌우 사다리를 확인.
                    if (tv_cols_tag == border_lbr) {
                        if (vec_switch) {
                            persons_cols++;
                        } else {
